...
export const stockEntrySchema=z.object({productId:z.uuid('Selecione um produto'),productVariantId:z.string(),type:z.enum(['INITIAL','PURCHASE','RETURN']),quantity:quantity.refine(value=>Number(value.replace(',','.'))>0,'A quantidade deve ser maior que zero'),reason:z.string().trim().min(3,'Informe uma justificativa').max(500),referenceId:z.string().trim().max(120,'Use no máximo 120 caracteres')})
