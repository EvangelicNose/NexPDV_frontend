...
import { ArrowLeft, CheckCircle2, Clock3, CreditCard, RefreshCw, UserRound } from 'lucide-react'
...
export function OrderDetailsPage() {
  ...
  const checkoutSuccess = (sale: Sale) => {
    queryClient.setQueryData<Order>(['order', id], current => current ? {
      ...
      sale: { id: sale.id, sequence: sale.sequence, status: sale.status, total: sale.total, createdAt: sale.createdAt, payments: sale.payments },
    } : current)
  ...
  return <>
    <section className="order-details page-enter">
      ...
      <div className="detail-heading">
        ...
        <div className="detail-heading-actions"><span className={`order-status ${status.className}`}>{status.label}</span>{canCheckout ? <button className="checkout-open-button" onClick={() => setCheckoutOpen(true)}><CreditCard size={17}/> Receber e finalizar</button> : !order.sale && order.status !== 'DELIVERED' ? <OrderAdvanceButton order={order}/> : null}</div>
      </div>
      <div className="detail-grid">
        ...
        <aside className="detail-side">
          ...
          {order.sale ? <section className="panel sale-completed-card"><header><span><CheckCircle2 size={18}/></span><div><strong>Venda #{String(order.sale.sequence).padStart(4, '0')}</strong><small>Concluída em {date(order.sale.createdAt)}</small></div></header><div>{order.sale.payments.map(payment => <p key={payment.id}><span>{paymentMethodLabels[payment.method]}</span><strong>{money(payment.netAmount)}</strong></p>)}</div><footer><span>Total líquido recebido</span><strong>{money(String(order.sale.payments.reduce((sum, payment) => sum + Number(payment.netAmount), 0)))}</strong></footer></section> : <section className="panel detail-time"><Clock3 size={17}/><span><strong>Status atual</strong><small>{status.label}</small></span></section>}
        </aside>
      </div>
    </section>
    {checkoutOpen && <CheckoutModal orderId={order.id} total={order.total} onClose={() => setCheckoutOpen(false)} onSuccess={checkoutSuccess}/>}
  </>
}
