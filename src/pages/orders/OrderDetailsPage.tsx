    ...
    <section className="order-details page-enter">
      ...
      <div className="detail-grid">
        ...
        <aside className="detail-side">
          ...
          {order.sale ? <section className="panel sale-completed-card"><header><span><CheckCircle2 size={18}/></span><div><strong>Venda #{String(order.sale.sequence).padStart(4, '0')}</strong><small>Concluída em {date(order.sale.createdAt)}</small></div></header><div>{order.sale.payments.map(payment => <p key={payment.id}><span>{paymentMethodLabels[payment.method]}</span><strong>{money(payment.amount)}</strong></p>)}</div><footer><span>Total recebido</span><strong>{money(order.sale.total)}</strong></footer></section> : <section className="panel detail-time"><Clock3 size={17}/><span><strong>Status atual</strong><small>{status.label}</small></span></section>}
        </aside>
      </div>
    </section>
